...
void Renderer::Triangles(Point2 &vertex1, Point2 &vertex2, Point2 &vertex3)
{
    ...
    for (int x = xMin; x <= xMax; x++)
        for (int y = yMin; y <= yMax; y++)
        {
            ...
            {
                temp.color.r = vertex1.color.r * alpha + vertex2.color.r * beta + vertex3.color.r * gama;
                temp.color.g = vertex1.color.g * alpha + vertex2.color.g * beta + vertex3.color.g * gama;
                temp.color.b = vertex1.color.b * alpha + vertex2.color.b * beta + vertex3.color.b * gama;
                temp.color.a = vertex1.color.a * alpha + vertex2.color.a * beta + vertex3.color.a * gama;
                Pixel(temp);
            }
        }
}
